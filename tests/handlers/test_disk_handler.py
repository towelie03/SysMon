...
def mock_disk_partitions(monkeypatch):
    ...
    def _mock():
        return [
            ...
            psutil._common.sdiskpart(device='/dev/sdb1', mountpoint='/data', fstype='ntfs', opts='ro'),
        ]
...
def mock_disk_usage(monkeypatch):
    ...
    def _mock(path):
        if path == '/':
            ...
            return psutil._common.sdiskusage(total=100000000000, used=100000000000, free=0, percent=100.0)
...
def mock_disk_io_counters(monkeypatch):
    ...
    def _mock(perdisk=False):
        if perdisk:
            return {
                'sda': psutil._common.sdiskio(read_count=1000, write_count=500, read_bytes=1048576, write_bytes=524288,
                                              read_time=10, write_time=20),
                'sdb': psutil._common.sdiskio(read_count=200, write_count=100, read_bytes=2048, write_bytes=1024,
                                              read_time=5, write_time=10),
            }
        return psutil._common.sdiskio(read_count=1200, write_count=600, read_bytes=1048578, write_bytes=524290,
                                      read_time=15, write_time=30)
...
def mock_disk_io_counters_no_io(monkeypatch):
    ...
    def _mock(perdisk=False):
        if perdisk:
            return {
                'sda': psutil._common.sdiskio(read_count=0, write_count=0, read_bytes=0, write_bytes=0,
                                              read_time=0, write_time=0),
            }
        return psutil._common.sdiskio(read_count=0, write_count=0, read_bytes=0, write_bytes=0,
                                      read_time=0, write_time=0)
    monkeypatch.setattr(psutil, "disk_io_counters", _mock)
...
def test_get_disk_usage(mock_disk_usage):
    ...
    expected_root = {'total': 500000000000, 'used': 200000000000, 'free': 300000000000, 'percent': 40.0}
    expected_data = {'total': 100000000000, 'used': 100000000000, 'free': 0, 'percent': 100.0}
...
def test_get_disk_io_counters(mock_disk_io_counters):
    ...
    expected = {'read_count': 1200, 'write_count': 600, 'read_bytes': 1048578, 'write_bytes': 524290, 'read_time': 15, 'write_time': 30}
    assert result == expected
...
def test_get_disk_io_counters_per_disk(mock_disk_io_counters):
    ...
    expected = {
        'sda': {'read_count': 1000, 'write_count': 500, 'read_bytes': 1048576, 'write_bytes': 524288, 'read_time': 10, 'write_time': 20},
        'sdb': {'read_count': 200, 'write_count': 100, 'read_bytes': 2048, 'write_bytes': 1024, 'read_time': 5, 'write_time': 10},
    }
    assert result == expected

def test_get_disk_active_time_percentage(mock_disk_io_counters_for_active_time, mock_time, mock_sleep):
...
def test_get_disk_usage_no_space(mock_disk_usage):
    ...
    expected = {'total': 100000000000, 'used': 100000000000, 'free': 0, 'percent': 100.0}
    assert result == expected

def test_get_disk_io_counters_no_io(mock_disk_io_counters_no_io):
    """Test get_disk_io_counters with no I/O activity."""
    result = Disk.get_disk_io_counters()
    expected = {'read_count': 0, 'write_count': 0, 'read_bytes': 0, 'write_bytes': 0, 'read_time': 0, 'write_time': 0}
    assert result == expected
